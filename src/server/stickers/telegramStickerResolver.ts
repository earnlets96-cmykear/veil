...
export class TelegramStickerResolver {
  ...
  public static async resolvePack(
    ...
    // 1. Try Telegram Bot API if a bot token was supplied
    if (botToken && botToken.trim()) {
      try {
        const tgRes = await fetch(
          `https://api.telegram.org/bot${botToken.trim()}/getStickerSet?name=${encodeURIComponent(cleanName)}`,
        ...
        if (tgRes.ok) {
          ...
          if (data.ok && data.result) {
            ...
            const stickers: ResolvedStickerItem[] = rawStickers.map((s: any, idx: number) => ({
              ...
              url: `https://api.telegram.org/file/bot${botToken.trim()}/${s.file_path || s.file_id}`,
              width: s.width || 512,
              height: s.height || 512,
            }));
