...
describe('VEIL Phase 24: End-to-End Phone 1 ↔ Phone 2 UX & Real-World Validation Suite', () => {
  ...
  it('completes the full real-world discovery, 20-message chat, offline queueing, process rehydration, and username update sequence', async () => {
    const netConfig = {
    ...
    // 3. Step 1: Phone 1 searches @phone2
    const searchRes = await client.searchProfiles('phone2');
    expect(searchRes).toHaveLength(1);
    expect(searchRes[0].username).toBe('phone2');

    // 4. Step 2: Phone 1 sends contact request
    const p2Profile = (await client.getProfileByUsername('phone2'))!;
    await reqMgr1.sendContactRequest(s1, profile1, p2Profile, 'Hello Phone 2');

    // 5. Step 3: Phone 2 receives request
    ...
    // 6. Step 4: Phone 2 accepts
    await reqMgr2.acceptRequest(s2, p2Reqs[0].requestId, profile2);

    // 7. Step 5: Phone 1 receives acceptance
    ...
    // 8. Step 6: 20 Bidirectional Messages
    const cForP2 = (await contacts1.getContact(s1, doc2.identityId))!;
    const cForP1 = (await contacts2.getContact(s2, doc1.identityId))!;

    for (let i = 1; i <= 20; i++) {
      if (i % 2 !== 0) {
        const text = `Phone 1 message #${i}`;
        const { wirePayloadBase64 } = await conv1.encryptAndPackWireMessage(s1, cForP2.prekeyBundle!, text);
        await net1.sendEnvelope(s1, cForP2.mailboxId!, wirePayloadBase64);
        ...
        const { wirePayloadBase64 } = await conv2.encryptAndPackWireMessage(s2, cForP1.prekeyBundle!, text);
        await net2.sendEnvelope(s2, cForP1.mailboxId!, wirePayloadBase64);
    ...
    // 9. Step 7: Phone 2 updates username to @phone2_renamed
    const profile2Updated = createSignedProfile(doc2.identityId, id2.signingPrivateKey, 'phone2_renamed', 'Phone 2 Renamed', mb2.mailboxId, bundle2);
    await client.updateProfile(profile2Updated);

    // 10. Step 8: Post-rename messaging continues seamlessly
    const postRenameText = 'Message after Phone 2 changed username';
    const { wirePayloadBase64: postWire } = await conv1.encryptAndPackWireMessage(s1, cForP2.prekeyBundle!, postRenameText);
    await net1.sendEnvelope(s1, cForP2.mailboxId!, postWire);

    let postRenameRecv = '';
    await net2.syncMailbox(s2, async (p) => {
      const res = await conv2.processInboundWirePayload(s2, p);
      postRenameRecv = res.storedMessage.text;
    });
    expect(postRenameRecv).toBe(postRenameText);
  });
});
