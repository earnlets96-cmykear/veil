...
describe('VEIL Phase 23: Real-Device Discovery & Acceptance Contract Tests', () => {
  ...
  it('verifies full real-device discovery, contact request, acceptance, and messaging contract', async () => {
    const netConfig = {
    ...
    const search = await client.searchProfiles('phone2');
    expect(search).toHaveLength(1);
    expect(search[0].username).toBe('phone2');

    // 4. Step 2: Phone 1 fetches profile and sends request
    const p2Profile = await client.getProfileByUsername('phone2');
    await reqMgr1.sendContactRequest(s1, profile1, p2Profile!, 'Connecting from Phone 1');
    ...
    await reqMgr2.acceptRequest(s2, p2Requests[0].requestId, profile2);
    ...
    // 8. Step 6 & 7: Phone 1 sends E2EE message to Phone 2
    const cP2 = (await contacts1.getContact(s1, doc2.identityId))!;
    const msg1 = 'Hello from Phone 1';
    const { wirePayloadBase64: wire1 } = await conv1.encryptAndPackWireMessage(s1, cP2.prekeyBundle!, msg1);
    await net1.sendEnvelope(s1, cP2.mailboxId!, wire1);

    let p2Recv = '';
    await net2.syncMailbox(s2, async (payload) => {
      const res = await conv2.processInboundWirePayload(s2, payload);
      p2Recv = res.storedMessage.text;
    });
    expect(p2Recv).toBe(msg1);

    // 9. Step 8 & 9: Phone 2 replies to Phone 1
    const cP1 = (await contacts2.getContact(s2, doc1.identityId))!;
    const msg2 = 'Hello from Phone 2';
    const { wirePayloadBase64: wire2 } = await conv2.encryptAndPackWireMessage(s2, cP1.prekeyBundle!, msg2);
    await net2.sendEnvelope(s2, cP1.mailboxId!, wire2);

    let p1Recv = '';
    await net1.syncMailbox(s1, async (payload) => {
      const res = await conv1.processInboundWirePayload(s1, payload);
      p1Recv = res.storedMessage.text;
    });
    expect(p1Recv).toBe(msg2);
  });
});
